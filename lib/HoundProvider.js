    ...
    return (
      <div className="hound-result-item">
        ...
        <div className="hound-result-item context">
          <Line
            number={codeMatch.LineNumber - 2}
            content={codeMatch.Before[0]}
          />
          <Line
            number={codeMatch.LineNumber - 1}
            content={codeMatch.Before[1]}
          />
          <Line
            number={codeMatch.LineNumber}
            content={codeMatch.Line}
            query={item.query}
          />
          <Line
            number={codeMatch.LineNumber + 1}
            content={codeMatch.After[0]}
          />
          <Line
            number={codeMatch.LineNumber + 2}
            content={codeMatch.After[1]}
          />
        </div>
      </div>
    );
    ...
    return (
      <div className="line">
        ...
        <span className="line-content">
          {this.props.query
            ? <LineContent line={this.props.content} query={this.props.query} />
            : this.props.content}
        </span>
      </div>
    );
...
class LineContent extends React.Component {
  shouldComponentUpdate(nextProps) {
    return (
      this.props.query !== nextProps.query || this.props.line !== nextProps.line
    );
  }
  ...
  render() {
    return (
      <span className="line-content">
        <Highlight search={new RegExp(this.props.query)}>
          {this.props.line}
        </Highlight>
      </span>
    );
